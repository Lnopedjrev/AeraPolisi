...
from .models import (ProductOffers, ProductsGallery, Products,
                     OrderInfo, ShippingAdress, OrderRequests)
...
class Shop(DataMixin, ListView):
    ...
    def get_queryset(self) -> QuerySet[Any]:
        ...
        new_queryset = (ProductOffers
                        .objects
                        .filter(product__ontest=False, is_active=True)
...
class ProductCheckout(UserPassesTestMixin, DataMixin, FormView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        last_order = (OrderInfo
                      .objects
                      .filter(offer=offer, payee=customer)
                      .last())

        ship = None
        if not offer.product.service:
            ship, created = (ShippingAdress
                             .objects
                             .get_or_create(address=f_data['address'],
                                            order=last_order,
                                            city=f_data['city'],
                                            state=f_data['state'],
                                            zipcode=f_data['zipcode']))
        with transaction.atomic():
            orderreq, created = (OrderRequests
                                 .objects
                                 .get_or_create(
                                     shipping=ship,
