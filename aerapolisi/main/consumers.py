...
class NotificationConsumer(WebsocketConsumer):
    def connect(self):
        ...
        self.send(
            text_data=json.dumps(
                {"type": "connection_established", "messsage": "You are now connected"}
            )
        )
    ...
    def receive(self, text_data):
        ...
        async_to_sync(self.channel_layer.group_send)(self.room_group_name, data_to_send)

        if owner != self.room_group_name:
            async_to_sync(self.channel_layer.group_send)(owner, data_to_send)
